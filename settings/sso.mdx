...
When SSO is active, MFA is handled by Azure AD's conditional access policies — not by Regentra's own [passkeys or TOTP MFA](/settings/mfa). This means:
...
- Users who sign in via email/password (non-SSO) still use Regentra's built-in passkey or TOTP MFA
