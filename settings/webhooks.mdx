...
Click **Send Test** on any endpoint to fire a sample delivery immediately, signed exactly like a real one. A test send is recorded in the delivery log but never affects the endpoint's failure count or health status — it's purely diagnostic.
