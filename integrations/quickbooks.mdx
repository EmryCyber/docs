...
<Steps>
  ...
  <Step title="(Optional) Turn on auto-push for Stripe-imported invoices">
    If you also import invoices from Stripe (see [Stripe → Importing invoices created directly in Stripe](/integrations/stripe#importing-invoices-created-directly-in-stripe)), the mapping page has an "Auto-push to QuickBooks" card with an **Automatically push invoices to QuickBooks** checkbox, off by default. See [below](#auto-pushing-invoices-created-from-a-stripe-import) for exactly what it does.
  </Step>
</Steps>
...
If you invoice through Stripe directly and also use QuickBooks as your books of record, turning on **Automatically push invoices to QuickBooks** (Settings → Integrations → QuickBooks → Configure mapping → "Auto-push to QuickBooks", off by default) automatically pushes newly-imported Stripe invoices to QuickBooks — you don't have to click **Send to QuickBooks** on each one.
