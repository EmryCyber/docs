...
<Warning>
  Don't paste the full admin URL (e.g. `us2-XXXXXXX.ws01-securityeducation.com`) into the Region field. That's the admin portal, not the reporting API host — Regentra detects this specific mistake and refuses to test against it. You can also enter a full `*.securityeducation.com` or `*.proofpoint.com` reporting-API URL instead of a region code, but any other host is rejected the same way.
</Warning>
...
<AccordionGroup>
  ...
  <Accordion title="Region field error names your admin console URL">
    If you pasted your Proofpoint admin console URL into the Region field, Regentra catches it before testing and tells you so directly, suggesting the region code to use instead based on your tenant's URL prefix (`us`/`eu`/`ap`). Enter that region code and re-test — the integration does not auto-detect region from an admin URL.
  </Accordion>
