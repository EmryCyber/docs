...
For MSP customers operating multiple client tenants, Regentra also exposes a **per-company Sync With M365 button** on the PSA Company detail page (**PSA → CRM → Companies → [company]**). This pulls a fresh snapshot of users + devices for that one customer without waiting for the next 4-hour cycle.
...
Portal SSO is configured separately for each client. When a client's Entra tenant is connected, their end users can sign in to the support portal using their corporate Microsoft accounts. Enable this under **PSA → Configuration → Support Portal → Authentication** (or the **Support Portal** Settings tile) within the client's context.
