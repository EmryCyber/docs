...
| **Fail** | Control failed the test | Review and remediate; if you want to track it as a finding, log a [Compliance Issue](/compliance/audit-lifecycle) yourself and tag its source as "Test Failure" |
...
<AccordionGroup>
  ...
  <Accordion title="What happens when a test fails?">
    The test's status updates to Fail and the result is logged to its history — nothing is created automatically. If it's worth tracking as a finding, log a [Compliance Issue](/compliance/audit-lifecycle) yourself with source "Test Failure" and link it to the control.
  </Accordion>
