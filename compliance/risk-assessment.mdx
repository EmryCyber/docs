...
<Steps>
  ...
  <Step title="Start a new assessment (Setup)">
    Click **New Assessment**. Name the assessment, note who's conducting it, choose a **Methodology** — NIST SP 800-30 Rev 1 (recommended), NIST SP 800-66 Rev 2, HHS SRA Tool v3.6, ISO 27001 / ISO 27005, or a custom Qualitative Analysis — and set the next review date.
  </Step>
  <Step title="Define scope & assets">
    Describe which systems, applications, and locations are in scope (the default text is written for a HIPAA ePHI scope and can be edited for other frameworks), list the asset inventory, and optionally note any threats already known.
  </Step>
  <Step title="Review and create">
    Confirm the summary and click **Create Assessment**.
  </Step>
  <Step title="Add findings and score">
    From the assessment's Findings tab, add risk findings from the pre-populated threat library or as custom entries, rate each by likelihood and impact, attach supporting evidence, and define remediation plans with owners.
  </Step>
</Steps>
...
<Note>
  Regentra also accepts a fourth "Critical" tier on both likelihood and impact (for findings the operator wants to flag as effectively-certain, or catastrophic). Any row/column using Critical produces a result at least as high as the corresponding High row/column, never lower.
</Note>
...
<Warning>
  Do not let assessments go stale — outdated risk assessments are a common audit finding and regulatory citation.
</Warning>

Each assessment's next-review date is tracked and shown on the Compliance Calendar, so your team can plan the next assessment cycle in advance.
