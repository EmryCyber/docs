...
- **Generate audit evidence** — The recipient-level sent/opened/completed record proves your workforce understood and agreed to your policies
...
<Steps>
  ...
  <Step title="Choose recipients">
    Build your recipient list from your Entra-synced workforce roster:
    - **All workforce users** — a single toggle for everyone
    - **Groups** — target specific Microsoft Entra security groups, marking each as **Include** or **Exclude** so you can, for example, target one department while excluding a sub-team within it
    - **Individuals** — add or exclude specific people; service/shared-mailbox accounts are excluded automatically unless you explicitly turn them on
    - **Add manually** — add a name and email directly, for recipients outside the synced roster
  </Step>
...
| **Completed** | Every recipient has signed |
...
## Reviewing acknowledgment records

<Steps>
  <Step title="Open the campaign">
    In the Compliance module sidebar, click **Campaigns** and select the campaign name.
  </Step>
  <Step title="Review the recipient list">
    Each recipient row shows sent, opened, and completed timestamps — this is the full record for the campaign, viewable directly on the page.
  </Step>
  <Step title="Document the evidence">
    There's no built-in export for this report today. If you need a standalone artifact for an audit package, document the campaign (a screenshot or a written summary of the recipient list) and attach it to the relevant control in [Evidence Collection](/compliance/evidence-collection).
  </Step>
</Steps>

<Warning>
  Signed acknowledgments should be treated as immutable records — auditors verify the integrity of your evidence.
</Warning>
...
5. **Archive evidence** — after the campaign closes, document the recipient list for your audit package
