...
- **Log findings and issues** — When you create an issue, tag its source as the audit, [Control Tests](/compliance/control-tests), [Access Reviews](/compliance/access-reviews), or manual entry
...
<Note>
  When you create an issue, you can select its source (Audit, Test Failure, Access Review, or Manual) and optionally link it to a control, an audit period, and an assignee — but issues are always created manually; nothing generates them automatically from a failed [Control Test](/compliance/control-tests) or an [Access Review](/compliance/access-reviews) finding.
</Note>
