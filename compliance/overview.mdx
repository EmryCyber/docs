...
<AccordionGroup>
  ...
  <Accordion title="NIST CSF">
    NIST Cybersecurity Framework 2.0. Organized around six functions: Govern, Identify, Protect, Detect, Respond, and Recover.
  </Accordion>
  ...
  <Accordion title="SEC Regulation S-P">
    SEC Regulation S-P (17 CFR 248 Subpart A). Requires broker-dealers, investment advisers, and investment companies to safeguard customer information and provide breach notice — converges with FTC Safeguards, HIPAA, and GDPR wherever their controls overlap.
  </Accordion>
  <Accordion title="FINRA">
    Financial Industry Regulatory Authority requirements for broker-dealers — supervision and written supervisory procedures, records retention (including WORM/17a-4), communications capture, cybersecurity program, identity-theft prevention (Reg S-ID), business continuity, and AML.
  </Accordion>
</AccordionGroup>

Regentra supports **11 frameworks** in total, all built on the same Common Control Framework so implementation work overlaps automatically wherever the requirements do.
...
Each "Control Progress by Domain" card can be expanded in place — click the caret to see the individual controls (code, title, status) in that domain without leaving the dashboard. A search box above the domain cards narrows the expanded list to matching controls and automatically expands any domain with a match.
