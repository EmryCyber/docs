...
<Steps>
  <Step title="Navigate to SLA Policies">
    In the PSA module sidebar, click **SLA Policies**, then click **Create Policy**.
  </Step>
...
- **Waiting Approval** — the ticket is on hold for an [approval decision](/psa/tickets#ticket-approvals). The timer resumes when the approver responds (or the request expires and the ticket returns to its prior status).
