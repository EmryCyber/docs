...
<Tip>
  You can also log time in bulk from **PSA → Finance → Time Sheets** for work that spans multiple tickets or non-ticket activities.
</Tip>
...
Service agreements are configured per company under **PSA → CRM → Companies → [company name] → Service Agreement**.
